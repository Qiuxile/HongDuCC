...
const ctnDom: Ref<HTMLDivElement | null> = ref(null)
...
onMounted(() => {
  ...
  const colorStopsArray: number[][] = props.colorStops.map((hex: string) => {
  ...
  program = new Program(gl, {
    ...
    uniforms: {
      ...
      uVerticalOffset: { value: props.verticalOffset ?? 0.2 },
    },
  })
  ...
  const update = (t: number): void => {
    ...
    if (program && renderer) {
      ...
      program.uniforms.uVerticalOffset.value = props.verticalOffset ?? 0.2
