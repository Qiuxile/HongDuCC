...
<template>
  <div
    ref="containerRef"
    :class="['gradual-blur', config.className]"
  ...
  >
    <div class="gradual-blur-inner">
      <div v-for="(div, index) in blurDivs" :key="index" class="gradual-blur-layer" :style="div.style" />
    </div>
    <div v-if="$slots.default" class="gradual-blur-slot">
      <slot />
    </div>
  </div>
</template>
